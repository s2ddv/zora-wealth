...
#[derive(Serialize)]
struct Checks { database: &'static str, redis: &'static str }
...
async fn health(State(state): State<HealthState>) -> Json<HealthResponse> {
    ...
    Json(HealthResponse {
        ...
        checks: Checks { database: if database { "ok" } else { "error" },
...
pub fn health_router(state: HealthState) -> Router {
    Router::new().route("/health", get(health))
        .route("/v1/health", get(health)).route("/v1/health/", get(health))
        .with_state(state)
}
